...
- `get_manipulator.m` which takes as input the definition of the manipulator structure (through the variables `DHParams`, `jointTypes` and `base`) and outputs a struct containing the symbolic definitions needed for kinematics
- `add_manipulator_dynamics.m` which takes as input the already computed struct and definition of the manipulator mass (through the variables `coms`, `masses`, `Is` and `g0`). It adds to the struct the symbolic definitions needed for dynamics
- `export_functions.m` which saves the symbolic functions to matlab code. It takes as input the robot struct, a string to prefix to the output files and a true/false flag telling the symbolic toolbox whether to optimize the code (slower while exporting)
...
- `q_dot`: vector of *symbolic variables* matlab objects that represent the time derivative of each joint coordinate and can be used to substitute with numerical values the `q_dot` symbols in the symbolic expression of `Ja_dot`
